...
const Product = require('../models/customizationProductModel');
const fs = require('fs');
...
const listProducts = async (req, res) => {
  try {
    ...
    res.render('customizationProduct/index', { products });
  } catch (error) {
...
const renderCreateForm = (req, res) => {
  res.render('customizationProduct/create');
};
...
const createProduct = async (req, res) => {
  try {
    ...
    const imagePath1 = 'customizationUploads/' + image1.name;
    const imagePath2 = 'customizationUploads/' + image2.name;
    ...
    res.redirect('/admin/customizationProduct');
  } catch (error) {
...
const renderEditForm = async (req, res) => {
  try {
    ...
    res.render('customizationProduct/edit', { product });
  } catch (error) {
    ...
    if (req.files && req.files.image1 && req.files.image2) {
      ...
      const image2 = req.files.image2;
        ...
        if (product.image1 && product.image2) {
          ...
          fs.unlinkSync(path.join(__dirname, '../public', product.image2));
        }
...
const renderDeleteForm = async (req, res) => {
  try {
    ...
    res.render('customizationProduct/delete', { product });
  } catch (error) {
